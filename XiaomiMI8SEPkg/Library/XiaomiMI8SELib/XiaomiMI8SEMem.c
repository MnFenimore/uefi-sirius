...
STATIC struct XiaomiMI8SEReservedMemory {
  ...
  )
{
  ...
  Count = sizeof (XiaomiMI8SEReservedMemoryBuffer) / sizeof (struct XiaomiMI8SEReservedMemory);
  while ((NextHob.Raw = GetNextHob (EFI_HOB_TYPE_RESOURCE_DESCRIPTOR, NextHob.Raw)) != NULL)
  {
         ...
         NextHob.ResourceDescriptor->PhysicalStart + NextHob.ResourceDescriptor->ResourceLength))
    {
      ...
      ReservedTop = XiaomiMI8SEReservedMemoryBuffer[Index].Offset + XiaomiMI8SEReservedMemoryBuffer[Index].Size;

      // Create the System Memory HOB for the reserved buffer
      BuildResourceDescriptorHob (
        EFI_RESOURCE_MEMORY_RESERVED,
        EFI_RESOURCE_ATTRIBUTE_PRESENT,
        XiaomiMI8SEReservedMemoryBuffer[Index].Offset,
        XiaomiMI8SEReservedMemoryBuffer[Index].Size
      );
      // Update the HOB
      NextHob.ResourceDescriptor->ResourceLength = XiaomiMI8SEReservedMemoryReservedMemoryBuffer[Index].Offset -
